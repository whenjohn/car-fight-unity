...
namespace cakeslice.SimpleWebRTC
{
	internal sealed class Connection : IDisposable
	{
		...
		public Connection(List<Common.ICEServer> _iceServers, ConnectionId connId, int maxMessageSize, string clientAddress, Action<Connection> onDispose)
		{
			...
			// A peer connection may report Connected before its data channels are open.
			// If FishNet queued its authentication response during that window, the send
			// loop already consumed the pending signal and the packet could otherwise sit
			// in the queue forever. Wake each loop again when its channel is truly ready.
			this.unreliableDataChannel.OnOpen += () =>
			{
				unreliableSendPending.Set();
			};
			this.reliableDataChannel.OnOpen += () =>
			{
				reliableSendPending.Set();
			};
