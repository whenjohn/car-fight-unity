				...
				while (client.ConnectionState == RTCPeerConnectionState.Connected)
				{
					...
					if (stream.ReadyState == RTCDataChannelState.Open)
					{
						while (sendQueue.TryDequeue(out ArrayBuffer msg))
						{
							...
							try
							{
								// Unity 6's WebRTC objects must be touched from Unity's main
								// thread. Signaling was already dispatched there, but payload
								// sends were still happening on this worker thread and could
								// silently strand FishNet's authentication response.
								Common.RunOnMainThread(() => stream.Send(pointer, length));
							}
							finally
							{
								pinned.Free();
